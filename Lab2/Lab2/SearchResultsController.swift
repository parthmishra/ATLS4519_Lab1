...
class SearchResultsController: UITableViewController, UISearchResultsUpdating {
    ...
    func updateSearchResultsForSearchController(searchController: UISearchController) {
        ...
        filteredClothes.removeAll(keepCapacity: true)
        if searchString?.isEmpty == false {
            ...
            for key in clothes {
                let wordsForKeys = allclothes[key]!
                let matches = wordsForKeys.filter(filter)
                filteredClothes.appendContentsOf(matches)
            }
        }
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = filteredClothes[indexPath.row]
        return cell
    }
